  ...
  describe "validation" do
    ...
    context "新規投稿に成功する場合" do
      ...
      it "PNG画像を追加した場合、投稿に成功すること" do
        image = build(:image, image_file: Rack::Test::UploadedFile.new(File.join(Rails.root, "spec/fixtures/test_png.png")))
        expect(image).to be_valid
      end

      it "jpeg画像を追加した場合、投稿に成功すること" do
        image = build(:image, image_file: Rack::Test::UploadedFile.new(File.join(Rails.root, "spec/fixtures/test_jpeg.jpeg")))
        expect(image).to be_valid
      end

      it "gif画像を追加した場合、投稿に成功すること" do
        image = build(:image, image_file: Rack::Test::UploadedFile.new(File.join(Rails.root, "spec/fixtures/test_gif.gif")))
        expect(image).to be_valid
      end
    end
  end
